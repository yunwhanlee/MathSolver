...
//-------------------------------------------------------------------------------------------------------------
    ...
    IEnumerator coDisplayQuestion(string title, string qstEquation, string qstCorrectAnswer, string qstWrongAnswers) {
        ...
        Debug.Log($"coDisplayQuestion:: curQuestionIndex= {curQuestionIndex},  map.childCount= {map.childCount}");
...
//-------------------------------------------------------------------------------------------------------------
    ...
    public IEnumerator coSuccessAnswer(int idx) {
        ...
        //* コンボ BurningEF
        if(GM._.ComboCnt >= Config.COMBO_ACTIVE_CNT) {
            GM._.gem.activeComboEF();
            if(!GM._.gem.PlBurningEF.activeSelf) GM._.gem.PlBurningEF.SetActive(true);
        }
