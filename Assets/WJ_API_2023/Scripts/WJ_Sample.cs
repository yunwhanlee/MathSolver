...
//-------------------------------------------------------------------------------------------------------------
    ...
    private void GetDiagnosis() {
        ...
        switch (wj_connector.cDiagnotics.data.prgsCd) {
            case "W":
                MakeQuestion(wj_connector.cDiagnotics.data.textCn,
    ...
    private void GetLearning(int idx) {
        ...
        MakeQuestion(wj_connector.cLearnSet.data.qsts[idx].textCn,
    ...
    //* 받아온 데이터를 가지고 문제를 표시
    /// </summary>
    private void MakeQuestion(string title, string qstEquation, string qstCorrectAnswer, string qstWrongAnswers) {
        Debug.Log("WJ_Sample:: MakeQuestion(title, qstEquation, qstCorrectAnswer, qstWrongAnswers)::"
            + "\n title= " + title //* タイトル
