...
public class HomeTalkManager : TalkManager {
    ...
    [SerializeField] Sprite zhenyi8Spr;
    [SerializeField] Sprite zhenyi9Spr;
    [SerializeField] Sprite zhenyi10Spr;
    [SerializeField] Sprite zhenyi11Spr;
    ...
    public override void generateData() {
        ...
        talkDt.Add((int)ID.ToZhenyi, new string[] {
            $"안녕하세요 zhengyi!:{(int)SPK.Pl_Idle}",
            $"동물마을의 해결사 늑선생이에요!:{(int)SPK.Pl_Idle}",
            $"우리 주인님이 Zhengyi를 너무 좋아하는데, 표현이 서툴러서:{(int)SPK.Pl_Idle}",
            $"제가 대신 전달을 해드리려고 해요.:{(int)SPK.Pl_Idle}",
            //* Zhenyi1 (4)
            $"외롭게 홀로 열심히 살고있는 주인님은 어느날 우연히 zhengyi를 봤데요.:{(int)SPK.Pl_Idle}",
            $"흰색드래스옷을 입은 zhengyi가 마치 천사같았어요.:{(int)SPK.Pl_Idle}",
            $"하지만 용기가없어서 말을 못걸고 그렇게 헤어지고 후회했어요.:{(int)SPK.Pl_Sad}",
            //* Zhenyi2 (7)
            $"그런데 다음날 운명처럼 공원에서 다시 zhengyi를 보게 되었어요.:{(int)SPK.Pl_Idle}",
            $"그때 주인님은 용기를내서, 연락처를 물어봤어요.:{(int)SPK.Pl_Idle}",
            //* Zhenyi3 (9)
            ...
            //* Zhenyi4 (16)
            $"그래서 zhengyi가 방콕에 가는걸 듣고:{(int)SPK.Pl_Idle}",
            $"따라가서 같이 시간보내고, 좋은 풍경에서 고백을 하고 싶었데요.:{(int)SPK.Pl_Idle}",
            ...
            $"이 힘든 세상에서 내가 멋있어지고, 성공한 사람이 되야한다고 생각했어요.:{(int)SPK.Pl_Idle}",
            //* Zhenyi6 (26)
            $"그렇게 운동을 시작하고, 오로지 일과 게임 개발만 하기 시작햇어요.:{(int)SPK.Pl_Idle}",
            $"그리고 1월달에 야구게임을 출시했어요.:{(int)SPK.Pl_Idle}",
            $"문득 LINE에 zhengyi의 연락처가 남아있는게 생각이나서.:{(int)SPK.Pl_Idle}",
            ...
            $"zhengyi가 게임대회에도 한 번 출전해보라고 말했어요.:{(int)SPK.Pl_Idle}",
            $"주인님은 그렇게 대회에 나가서 우승하면, zhengyi가 나에게 관심을 가져줄거라고 생각했어요.:{(int)SPK.Pl_Idle}",
            ...
            $"zhengyi를 많이 좋아하고 즐겁게 해주고싶은데,:{(int)SPK.Pl_Idle}",
            $"zhengyi씨의 마음은 어떤가요?:{(int)SPK.Pl_Idle}",
            $"승낙하신다면, 옆에 있는 남자의 손을 꼭 잡아주세요!:{(int)SPK.Pl_Idle}",
        });
        #endregion
    }
