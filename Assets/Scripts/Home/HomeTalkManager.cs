...
public class HomeTalkManager : TalkManager {
    ...
    public override void generateData() {
        ...
        talkDt.Add((int)ID.UNLOCK_MAP2_BG3_REWARD, new string[] {
            ...
            , $"자 그럼 이제 원숭이가 있는 신전을 알려줘야지?:{(int)SPK.Pl_Idle}"
        ...
        talkDt.Add((int)ID.OPEN_MAP3_UNLOCK_BG1_REWARD, new string[] {
            ...
            , $"과격하게 또때튀를 한건 미안하끼.. 나에겐 시간이 별로 없었다끼..:{(int)SPK.Monkey_God}"
            , $"아닙니다.. 정글을 위해서 하신건데요.:{(int)SPK.Pl_Idle}"
            , $"크허어엉 몽키신님..:{(int)SPK.WarriorMonkey_Sad}_FLIP"
